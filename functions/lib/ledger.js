...
exports.settlecredit = exports.recordsale = void 0;
/**
 ...
 */
async function assertStoreAccess(request, requestedStoreId) {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "You must be signed in to update the ledger.");
    }
    const uid = request.auth.uid;
    const claims = request.auth.token || {};
    const role = claims.role;
    const claimsStoreId = claims.storeId;
    // Resolve the target store: prefer the caller's claims, then the payload.
    const targetStoreId = requestedStoreId || (claimsStoreId && claimsStoreId !== "PLATFORM" ? claimsStoreId : undefined);
    if (!targetStoreId) {
        throw new https_1.HttpsError("invalid-argument", "Unable to determine store. Refresh and try again.");
    }
    if (role === "system_admin")
        return targetStoreId;
    if (claimsStoreId && claimsStoreId !== "PLATFORM" && claimsStoreId === targetStoreId)
        return targetStoreId;
    ...
    if (storeSnap.exists && storeSnap.data()?.ownerId === uid)
        return targetStoreId;
    throw new https_1.HttpsError("permission-denied", "You can only record ledger changes for your assigned store. Contact your administrator.");
}
/**
 ...
 */
exports.recordsale = (0, https_1.onCall)({ cors: true }, async (request) => {
    ...
    const recordedByName = token.name || token.email || "Staff";
    ...
    try {
        await db.runTransaction(async (tx) => {
            ...
            for (const item of items) {
                ...
                if (!snap.exists) {
                    throw new https_1.HttpsError("not-found", `Product not found: ${item.itemId}`);
                }
                const product = snap.data();
                if (product.storeId !== storeId) {
                    throw new https_1.HttpsError("failed-precondition", `Product ${item.itemId} belongs to a different store — cannot update its inventory.`);
                }
                const delta = item.quantity * (item.conversionFactor || 1);
                if (!restock) {
                    const available = Number(product.currentStock) || 0;
                    if (available < delta) {
                        throw new https_1.HttpsError("failed-precondition", `Insufficient stock for ${product.name || item.itemId}: only ${available} available, ${delta} required.`);
                    }
                }
            ...
            for (const { item, delta } of productSnaps) {
                tx.update(db.collection("products").doc(item.itemId), {
                    currentStock: firestore_1.FieldValue.increment(restock ? delta : -delta),
                    updatedAt: now,
                });
        ...
        throw new https_1.HttpsError("internal", "Failed to record the sale. Please try again.");
    }
});
/**
 ...
 */
exports.settlecredit = (0, https_1.onCall)({ cors: true }, async (request) => {
    const data = (request.data || {});
    const phone = (data.customerPhone || "").trim();
    if (!phone || phone.length < 6) {
        throw new https_1.HttpsError("invalid-argument", "A valid customer phone number is required.");
    }
    const amount = Math.abs(Number(data.amountNgn) || 0);
    if (amount <= 0) {
        throw new https_1.HttpsError("invalid-argument", "A top-up amount greater than zero is required.");
    }
    const storeId = await assertStoreAccess(request, data.storeId);
    const uid = request.auth.uid;
    const token = request.auth.token || {};
    ...
    debtsSnap.forEach((d) => {
        const x = d.data();
        if (x.customerPhone === phone)
            debt += Number(x.amountNgn) || 0;
    });
    ...
    console.log(`settlecredit: ${phone} topped up ${fmt(amount)} for store ${storeId} — ${fmt(clearedDebt)} cleared debt, ${fmt(toCredit)} credited`);
