...
export interface PaystackTransferRecipientOptions {
  type?: "nuban";
  name: string;
...
export interface PaystackInitiateTransferOptions {
  source?: "balance";
  amountInKobo: number;
...
export class PaystackService {
   ...
   */
  public static async initializeTransaction(options: PaystackInitializeOptions): Promise<{
    ...
    if (options.channels) payload.channels = options.channels;
    if (options.bearer) payload.bearer = options.bearer;
   ...
   */
  public static async verifyTransaction(reference: string): Promise<{
  ...
  public static async resolveAccountNumber(accountNumber: string, bankCode: string): Promise<{
    ...
    const response = await fetch(
      `${this.BASE_URL}/bank/resolve?account_number=${encodeURIComponent(accountNumber)}&bank_code=${encodeURIComponent(bankCode)}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${secret}`,
   ...
   */
  public static async listBanks(): Promise<Array<{ name: string; code: string; slug: string; id: number }>> {
    const secret = this.getSecretKey();
   ...
   */
  public static async createTransferRecipient(options: PaystackTransferRecipientOptions): Promise<{
    recipientCode: string;
    name: string;
    accountNumber: string;
    bankCode: string;
   ...
   */
  public static async initiateTransfer(options: PaystackInitiateTransferOptions): Promise<{
    transferCode: string;
    reference: string;
